...
function buildNft(index: number): Nft {
  ...
  const soldRatio = rnd();
  const editionsAvailable = Math.max(
    0,
    Math.round(editionsTotal * (1 - soldRatio * 0.8)),
  );
  ...
  return {
    ...
    tags: [
      ...
      collectionTag(rnd),
      rnd() > 0.5 ? "Edição limitada" : "1 de 1",
    ],
    creator,
    ...
    images: [0, 1, 2, 3].map(
      () => `/assets/nft/nft-${String((index % 4) + 1).padStart(3, "0")}.svg`,
    ),
    coverImage: `/assets/nft/nft-${String((index % 4) + 1).padStart(3, "0")}.svg`,
    priceEth,
    lastSalePriceEth: hasLastSale
      ? (basePrice * (0.85 + rnd() * 0.3)).toFixed(3)
      : null,
    editionsTotal,
    editionsAvailable,
    ...
    attributes: [
      ...
      {
        trait: "Complexidade",
        value: ["Baixa", "Média", "Alta"][Math.floor(rnd() * 3)],
      },
